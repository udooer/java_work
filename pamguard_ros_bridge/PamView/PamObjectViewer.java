...
public class PamObjectViewer implements PamViewInterface, ComponentListener,
	...
	Color getProcessColour(PamProcess pamProcess) {
		...
		if (pamProcess.getBackendProcess() != null) {
			return Color.GREEN;
		}
		return Color.RED;
	}

	Color getDataBlockColour(PamProcess pamProcess, PamDataBlock pamDataBlock) {
		if (pamProcess == null || pamDataBlock == null) {
			return null;
		}
		BackendProcess backEndProcess = pamProcess.getBackendProcess();
		if (backEndProcess == null) {
			return null;
		}
		if (pamDataBlock != backEndProcess.getOutputDataBlock()) {
			return null;
		}
		if (pamProcess.getBackendProcess().getReturnData(pamDataBlock) == false) {
			return Color.white;
		}
		return null;
	}
