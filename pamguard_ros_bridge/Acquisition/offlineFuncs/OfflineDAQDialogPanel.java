...
public class OfflineDAQDialogPanel {
	 ...
	 */
	public OfflineDAQDialogPanel(AcquisitionControl acquisitionControl,
	...
	public void setParams() {
		OfflineFileParameters p = acquisitionControl.getOfflineFileServer().getOfflineFileParameters();
	...
	public OfflineFileParameters getParams() {
		...
		if (checkFolder(p.folderName) == false && p.enable) {
			if (p.folderName == null) {
				acquisitionDialog.showWarning("Error in file store", "No storage folder selected");
				return null;
			}
			else {
				String err = String.format("The folder %s does not exist", p.folderName);
				acquisitionDialog.showWarning("Error in file store", err);
				return null;
			}
		}
		return p;
