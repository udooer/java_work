...
public class FolderInputSystem extends FileInputSystem implements PamSettings{
	...
	public FolderInputSystem() {
		...
		audioFileFilter = new PamFileFilter("Audio Files", ".wav");
		audioFileFilter.addFileType(".WAV");
		audioFileFilter.addFileType(".aif");
		audioFileFilter.addFileType(".aiff");
		audioFileFilter.addFileType(".AIF");
		audioFileFilter.addFileType(".AIFF");
		makeSelFileList();
...
//				fileSamples = fileLength / audioFormat.getFrameSize();
	...
	public void setFileDateText() {
		if (allFiles.size() > 0) {
			long fileTime = fileDate.getTimeFromFile(getCurrentFile());
	...
	protected boolean openNextFile() {
		...
		if (++currentFile < allFiles.size()) {
			...
			fileEndTime += lastBit;
			long newStartTime = fileDate.getTimeFromFile(getCurrentFile());
