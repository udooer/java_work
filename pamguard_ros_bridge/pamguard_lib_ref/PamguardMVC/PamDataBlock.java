...
public class PamDataBlock<Tunit extends PamDataUnit> extends PamObservable {
	 ...
	 * @param channels channel bitmap of data unit
	...
	public boolean loadViewerData(long dataStart, long dataEnd) {
		...
		return dataSource.loadData(this, dataStart, dataEnd, null);
	...
	public void updatePamData(Tunit pamDataUnit, long timeMillis) {
		pamDataUnit.updateDataUnit(timeMillis);
		setChanged();
		if (PamController.getInstance().getRunMode() != PamController.RUN_PAMVIEW) {
			if (binaryDataSource != null) {
				binaryDataSource.saveData(pamDataUnit);
	...
	synchronized public Tunit getClosestUnitMillis(long startTime) {
		...
		//		for (int i = pamDataUnits.size() - 2; i >= 0; i--) {
		//			if ((newdifference = Math.abs(startTime
		//					- pamDataUnits.get(i).timeMilliseconds)) < difference) {
		//				difference = newdifference;
		//				unit = pamDataUnits.get(i);
