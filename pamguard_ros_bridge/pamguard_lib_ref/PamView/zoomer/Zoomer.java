...
//		menu.addSeparator();
	...
	private class ZoomerMouse extends MouseAdapter {
		...
		private void doubleClicked(MouseEvent e) {
			if (topMostShape != null) {
				if (topMostShape.isClosed() && 
						topMostShape.containsPoint(e.getComponent(), e.getPoint())) {
					zoomIn();
				}
				else {
					clearLatestShape();
		...
		private void singleClicked(MouseEvent e) {
			if (newZoomPolygon != null) {
				if (newZoomPolygon.getStartPoint().distance(e.getPoint()) < 6 && 
						newZoomPolygon.getNumPoints() >= 3) {
		...
		public void mouseDragged(MouseEvent e) {
			...
			if (newZoomRectangle == null) {
				...
				topMostShape = newZoomRectangle = new ZoomRectangle(zoomer, zoomableThing.getCoordinateType(), 
						pointXtoVal(mousePressPoint.x), pointYtoVal(e.getComponent(), mousePressPoint.y));
			}
			newZoomRectangle.newPoint(pointXtoVal(e.getX()), pointYtoVal(e.getComponent(), e.getY()));
			zoomableThing.zoomShapeChanging(newZoomRectangle);
		}
