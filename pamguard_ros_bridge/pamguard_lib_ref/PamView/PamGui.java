...
public class PamGui extends PamView implements WindowListener, PamSettings {
			...
			PamModelInterface pamModelInterface, int frameNumber)
	{
		...
		outerSidePanel.add(BorderLayout.NORTH, sidePanel);
		scrollingSidePanel = new PamScrollPane(outerSidePanel);
		//		PamColors.getInstance().registerComponent(scrollingSidePanel, PamColors.PamColor.BORDER);
		...
		centralPanel.add(BorderLayout.WEST, scrollingSidePanel);
...
//	 */
	...
	PamControlledUnit findControlledUnit(int tabNo)
	{
		...
		Component tabComponent = mainTab.getComponent(tabNo);
		...
		for (int i = 0; i < pamControllerInterface.getNumControlledUnits(); i++) {
			...
			if (unit.getTabPanel() != null && tabComponent == unit.getTabPanel().getPanel()) return unit;
		}
	...
	public JMenuBar makeGuiMenu() {
		...
		fileMenu.add(menuItem);
		if (!isViewer) {
			if (PamguardVersionInfo.getReleaseType() >= PamguardVersionInfo.RELEASE_SMRU) {
		...
		for (int i = 0; i < pamControllerInterface.getNumControlledUnits(); i++) {
			...
			//System.out.println("PamGui, unit name: " + pamControllerInterface.getControlledUnit(i).toString());

			menuItem = pamControllerInterface.getControlledUnit(i).createFileMenu(frame);

			//System.out.println("PamGui, menuItem: " + menuItem.toString());
			...
			if (menuItem != null) {
				...
				fileMenu.add(menuItem);
				//System.out.println("PamGui, adding menu item: " + pamControllerInterface.getControlledUnit(i).toString());
			}
		}
	...
	private void showSidePanel() {
		scrollingSidePanel.setVisible(sidePanel.getComponentCount() > 0);
	}
