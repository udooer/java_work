...
//	private long lastUnitSamples = 0;
	...
	/**
	 * Process which will work across the JNI to run in C prior to 
	 * code being passed to Java. 
	 */
...
//			if (acousticDataSource) {
//				AcousticDataUnit acousticDataUnit = (AcousticDataUnit) arg;
//				lastUnitMilliseconds = acousticDataUnit.timeMilliseconds;
//				lastUnitSamples = acousticDataUnit.getStartSample();
//			}
			newData(o, arg);
		}
		else {
...
//		}
	...
	public void setBackendProcess(BackendProcess backendProcess) {
		if (pamBuoyGlobals.isUseDSP()) {
			BackendManager bem = BackendManager.getInstance();
			if (bem.isLoadLibraryOK() == false) {
				return;
			}
			bem.addProcess(backendProcess);
	...
	public final void fillXMLElement(Document doc, Element processData, int procId) {
		...
		processData.setAttribute("BackendC", ((Boolean)(this.getBackendProcess() != null)).toString());
